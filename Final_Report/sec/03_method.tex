...
\begin{equation}
  ...
  \label{eq:detections}
\end{equation}
where $b_i = (x_{1i}, y_{1i}, x_{2i}, y_{2i})$ specifies bounding box coordinates, $c_i \in \mathcal{C}_{\text{COCO}}$ denotes the assigned COCO class category, and $s_i \in [0, 1]$ represents the detection confidence score. We restrict valid target detections to cat ($c_i = 15$) and dog ($c_i = 16$) class indices. Among all valid candidates, the system selects the spatially dominant target index $i^*$ according to maximum bounding box area:
\begin{equation}
...
\begin{equation}
  x_{\text{masked}}(u, v) = \begin{cases} 
  0, & \text{if } \exists j \neq i^* \text{ s.t. } (u, v) \in b_j, \\ 
  x(u, v), & \text{otherwise.} 
  \end{cases}
  \label{eq:masking}
\end{equation}
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lllrrr@{}}
    ...
    5 & Ragdoll & Oxford-IIIT & 1,096 & 274 & 1,370 \\
    6 & Maine Coon & Oxford-IIIT & 1,232 & 308 & 1,540 \\
    7 & Sphynx & Oxford-IIIT & 1,168 & 292 & 1,460 \\
    ...
    10 & Beagle & Stanford Dogs & 1,224 & 306 & 1,530 \\
    11 & Pug & Stanford Dogs & 1,104 & 276 & 1,380 \\
    12 & Boxer & Stanford Dogs & 1,184 & 296 & 1,480 \\
    13 & Shiba Inu & Stanford Dogs & 1,216 & 304 & 1,520 \\
    14 & Samoyed & Oxford / Stanford & 1,280 & 320 & 1,600 \\
    15 & Golden Retriever & Oxford / Stanford & 1,248 & 312 & 1,560 \\
    16 & German Shepherd & Stanford Dogs & 1,200 & 300 & 1,500 \\
    17 & Siberian Husky & Stanford / Curation & 1,312 & 328 & 1,640 \\
    18 & Dalmatian & Stanford Dogs & 1,264 & 316 & 1,580 \\
    19 & Rottweiler & Stanford Dogs & 1,296 & 324 & 1,620 \\
    \midrule
...
During training, spatial data augmentation is applied dynamically to input crops. In addition to random horizontal flipping ($p=0.5$) and Gaussian blurring ($p=0.2$), we employ \texttt{CropMix}, a customized spatial patch-regularization technique. \texttt{CropMix} extracts a subcrop $x_{\text{sub}}$ with side length fraction $r \sim \mathcal{U}(0.6, 0.9)$ relative to the original crop $x_{\text{crop}}$ (resulting in an area coverage of $r^2$), resizes $x_{\text{sub}}$ back to $224 \times 224$, and linearly blends it with the original crop:
\begin{equation}
